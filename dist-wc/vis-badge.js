...
function St(i) {
  return i && De(i.destroy) ? i.destroy : se;
}
...
function Ft(i, e, t, n, l, r, a, f, s, c, u, d) {
  ...
  for (w = _; w--; ) {
    ...
    let T = a.get(g);
    T ? E.push(() => T.p(C, e)) : (T = c(g, C), T.c()), k.set(g, x[w] = T), g in b && P.set(g, Math.abs(w - b[g]));
  }
  ...
  for (; h && _; ) {
    const C = x[_ - 1], g = i[h - 1], T = C.key, H = g.key;
    C === g ? (u = C.first, h--, _--) : k.has(H) ? !a.has(T) || D.has(T) ? I(C) : V.has(H) ? h-- : P.get(T) > P.get(H) ? (V.add(T), I(C)) : (D.add(H), h--) : (s(g, a), h--);
  }
  for (; h--; ) {
    const C = i[h];
    k.has(C.key) || s(C, a);
...
function N(i, e, t) {
  ...
  n && n.m(e, t), Be(() => {
    const r = i.$$.on_mount.map(ot).filter(De);
...
`;
  function x() {
    ...
    T.id = Ie, T.textContent = b, document.head.appendChild(T);
  }
  function k(g) {
  ...
  async function D() {
    if (!s || !u || !d) return;
    await Ct();
    const g = u.getBoundingClientRect(), T = d.getBoundingClientRect(), H = window.innerWidth, pe = window.innerHeight;
    t(4, h = Rt(r, g, T));
    let j = 0, L = 0;
    h === "top" ? (L = g.top - Q - T.height, j = g.left + g.width / 2 - T.width / 2) : h === "bottom" ? (L = g.bottom + Q, j = g.left + g.width / 2 - T.width / 2) : h === "left" ? (L = g.top + g.height / 2 - T.height / 2, j = g.left - Q - T.width) : (L = g.top + g.height / 2 - T.height / 2, j = g.right + Q), j = ke(j, ee, H - T.width - ee), L = ke(L, ee, pe - T.height - ee);
    const xe = g.left + g.width / 2, z = g.top + g.height / 2, mt = ke(xe - j - 4, 10, T.width - 10), vt = ke(z - L - 4, 10, T.height - 10);
...
function dn(i, e, t) {
  ...
  return i.$$set = (D) => {
    "badge" in D && t(0, u = D.badge), "variant" in D && t(1, d = D.variant), "tooltip" in D && t(2, h = D.tooltip), "interactive" in D && t(3, _ = D.interactive), "ringText" in D && t(10, w = D.ringText), "repeat" in D && t(11, b = D.repeat), "separator" in D && t(12, x = D.separator), "size" in D && t(4, k = D.size), "sealFontScale" in D && t(13, P = D.sealFontScale);
  }, i.$$.update = () => {
    i.$$.dirty & /*badge*/
